...
function findFirstSingleChar(str) {
  const charsToCounts = str.split('').reduce((acc, char, i) => {
    if (!acc[char]) acc[char] = {};
    acc[char].origIndex = i;
    acc[char].count = (acc[char].count || 0) + 1;
    return acc;
  }, {});
  const singleChars = Object.keys(charsToCounts)
    .filter((x) => charsToCounts[x].count === 1)
    .sort((a, b) => charsToCounts[a].origIndex - charsToCounts[b].origIndex);
...
function isCreditCardNumber(ccn) {
  const digits = ccn.toString().split('').map((dig) => Number.parseInt(dig, 10));
...
function getDigitalRoot(num) {
  ...
  while (numStr.length > 1) {
    const sum = numStr.split('').map((ch) => Number.parseInt(ch, 10)).reduce((a, b) => a + b);
    numStr = sum.toString();
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const transposed = splitted[0].map((e, i) => splitted.map(elAt(i)));
  const common = transposed.filter(allElementsEqual).map(elAt(0));
  common.push('');
  return common.join('/');
}
...
function evaluateTicTacToePosition(position) {
  const checkWinerInTheRow = (rowRaw) => {
    const row = [];
    for (let i = 0; i < 3; i += 1) {
      row.push(rowRaw[i]);
    }
    const result = row.every((val) => val === row[0]);
  ...
  for (let i = 0; i < position.length; i += 1) {
    ...
    const winner = checkWinerInTheRow(row);
    if (winner) {
      return winner;
    }
  }
  ...
  for (let i = 0; i < position.length; i += 1) {
    ...
    diagonal2.push(position[i][position.length - i - 1]);
  }
  let winner = checkWinerInTheRow(diagonal1);
  if (winner) {
    return winner;
  }
  winner = checkWinerInTheRow(diagonal2);
  if (winner) {
    return winner;
  }
  return undefined;
